...
export const updateStatus = async (req, res) => {
    try {
        ...
        if (!staffUserId || isNaN(staffUserId))
            return res.json({ success: false, message: 'Missing or invalid staff user ID' })
        ...
        return res.json({ success: true, message: `Staff user is ${newStatus ? 'activated' : 'deactivated'} successfully` })
    } catch (error) {
        console.error(error);
        return res.json({ success: false, message: 'Failed to update staff user status. Please try again.' })
    }
}